...
/** 清理端口占用：Windows 用 netstat+taskkill，其他用 lsof+kill */
...
function startProcess({ name, command, args, logFileName }) {
  ...
  const child = spawn(command, args, {
    ...
    // Windows 上 .cmd 需经 shell 执行；非 Windows 保持 shell:false 更安全
    shell: IS_WIN,
    cwd: ROOT,
    env: process.env,
    // Windows 上 detached:true + pipe stdio 会报 EINVAL，仅在非 Windows 启用
    detached: !IS_WIN,
  });
